...
export default function FollowsPage() {
  ...
  const { updateFollowStatus, isLoading: isUpdating } = useUpdateFollowStatus();
  ...
  const renderContent = () => {
            ...
            searchResults.map((user) => {
              const follow =
                follows?.incoming.find(
                  (f) => f.sender?.id === user.id || f.recipient?.id === user.id
                ) ||
    ...
    if (activeTab === "blocked") {
      ...
      if (blocked.length === 0) {
        return (
          <div className="p-8 text-center">
            <p className="text-zinc-600 dark:text-zinc-400">No blocked users</p>
          </div>
        );
      }
      return (
    ...
    if (activeTab === "all") {
      ...
      if (accepted.length === 0) {
        return (
          <div className="p-8 text-center">
            <p className="text-zinc-600 dark:text-zinc-400">No friends yet</p>
          </div>
        );
      }
      return (
