...
passport.use(new JwtStrategy(jwtOpts, async (payload: JwtPayload, done) => {
    try {
...
passport.use(new GoogleStrategy(googleOpts, async (accessToken, refreshToken, profile, done) => {
    try {
